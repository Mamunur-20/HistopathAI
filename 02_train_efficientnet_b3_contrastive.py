...
def encoder_fn(input_shape):
    """
    Build and return an EfficientNetB3 encoder model with pre-trained weights.

    Parameters:
    - input_shape: Shape of the input image (height, width, channels)

    Returns:
    - Model: An encoder model based on EfficientNetB3
    """
    ...
    base_model = efn.EfficientNetB3(include_top=False, 
                                    ...
                                    pooling='avg')  # Pre-trained weights loaded here
    x = base_model(inputs)
...
for fold, (idxT, idxV) in enumerate(skf.split(np.arange(5))):
    ...
    oof_embed.append(encoder.predict(x_oof))  # Store embeddings for later analysis
