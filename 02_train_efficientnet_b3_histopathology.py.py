...
# Initialize lists to store predictions, labels, and embeddings for each fold
oof_reg_pred = []
oof_reg_labels = []
oof_reg_embed = []
...
for fold, (idxT, idxV) in enumerate(skf.split(np.arange(5))):
    ...
    # Calculate the step size and total steps for the training process
    ct_train = count_data_items(TRAIN_FILENAMES)
    step_size = (ct_train // BATCH_SIZE)
    total_steps = EPOCHS * step_size

    # Build the model within the distributed strategy scope
    with strategy.scope():
        encoder_reg = encoder_fn((None, None, CHANNELS))
        model_reg = classifier_fn((None, None, CHANNELS), N_CLASSES, encoder_reg)
        model_reg.summary()

        # Setup optimizer with cosine learning rate scheduling
    ...
    # Save the trained model's weights for the current fold
    ...
    # Make OOF (Out-Of-Fold) predictions
...
# Calculate the mean and standard deviation for precision, recall, F1-score, and accuracy
...
# Plot and save the training and validation metrics across all folds
plt.figure(figsize=(12, 6))
colors = ['b', 'g', 'r', 'c', 'm', 'y', 'k']  # Colors for the folds
...
# Visualize embeddings and confusion matrix
...
# Visualize embeddings using TSNE
visualize_embeddings(embeddings_reg, y_reg_true, filename="my_plot.png")

# Print final classification report for all folds
