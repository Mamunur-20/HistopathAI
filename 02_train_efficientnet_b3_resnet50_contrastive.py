...
BATCH_SIZE = 32 * REPLICAS  #  64 * REPLICAS, Batch size 64 works well, Batch size is scaled based on the number of replicas (GPUs)
...
# Function to create the EfficientNet-B3 encoder
def encoder_fn_efficientnet(input_shape):
    """
    Creates an EfficientNet-B3 encoder model.

    Parameters:
    input_shape -- tuple, the shape of the input image (height, width, channels)

    Returns:
    model -- Keras Model object, EfficientNet-B3 without the top layer and with ImageNet pre-trained weights
    """
    inputs = L.Input(shape=input_shape, name='inputs')
    base_model = efn.EfficientNetB3(include_top=False, 
                                    weights="imagenet", 
                                    pooling='avg')  # Using global average pooling
    x = base_model(inputs)
    model = Model(inputs=inputs, outputs=x)
    return model


# Function to create the ResNet50 encoder
def encoder_fn_resnet(input_shape):
    """
    Creates a ResNet50 encoder model.

    Parameters:
    input_shape -- tuple, the shape of the input image (height, width, channels)

    Returns:
    model -- Keras Model object, ResNet50 without the top layer and with ImageNet pre-trained weights
    """
    inputs = L.Input(shape=input_shape, name='inputs')
    base_model = ResNet50(weights="imagenet", include_top=False, pooling='avg')  # Using global average pooling
    x = base_model(inputs)
    model = Model(inputs=inputs, outputs=x)
    return model

# Function to create the classification model that combines the outputs of both encoders
def classifier_fn(input_shape, N_CLASSES, encoder1, encoder2, trainable=True):
    """
    Creates a classification model that combines the features from two encoder models (EfficientNet and ResNet).

    Parameters:
    input_shape -- tuple, the shape of the input image (height, width, channels)
    N_CLASSES -- int, the number of output classes
    encoder1 -- Keras Model, the first encoder model (EfficientNet)
    encoder2 -- Keras Model, the second encoder model (ResNet)
    trainable -- boolean, whether the encoder layers should be trainable or frozen during training

    Returns:
    model -- Keras Model object, the classification model that combines both encoders
    """
    # Set the layers of both encoders to be trainable or not
    ...
    features1 = encoder1(inputs)
    features2 = encoder2(inputs)
    
    # Concatenate the features from both models
    combined = L.concatenate([features1, features2])
    
    # Add fully connected (dense) layers for classification
    features = L.Dropout(0.5)(combined)  # Apply dropout for regularization
    features = L.Dense(1000, activation='relu')(features)  # Dense layer with ReLU activation
    features = L.Dropout(0.5)(features)  # Apply another dropout for regularization
...
# KFold Cross-Validation: Splitting the dataset into N folds for training and validation
skf = KFold(n_splits=N_FOLDS, shuffle=True, random_state=seed)

# Initializing empty lists to store out-of-fold predictions, labels, and embeddings
oof_pred = []
oof_labels = []
oof_embed = []
...
for fold, (idxT, idxV) in enumerate(skf.split(np.arange(5))):
    ...
    # Fetching training and validation filenames for the current fold
    ...
    # Shuffling the training filenames for better randomization
    np.random.shuffle(TRAIN_FILENAMES)
    
    # Counting the number of training images and calculating steps per epoch
    ct_train = count_data_items(TRAIN_FILENAMES)
    step_size = (ct_train // BATCH_SIZE)
    total_steps = EPOCHS * step_size

    ############### Pre-training the Encoder with Supervised Contrastive Loss ###############
    print('Pre-training the encoder using "Supervised Contrastive" Loss')
    
    with strategy.scope():
        # Initialize the encoder and add a projection head for contrastive learning
        encoder = encoder_fn((None, None, CHANNELS))
        encoder_proj = add_projection_head((None, None, CHANNELS), encoder)
        encoder_proj.summary()
        
        # Define the learning rate scheduler using cosine schedule with warmup
        ...
        # Compile the model with the Adam optimizer and supervised contrastive loss
        optimizer = optimizers.Adam(learning_rate=lr)
        encoder_proj.compile(optimizer=optimizer, loss=SupervisedContrastiveLoss(temperature))
    
    # Pre-train the encoder
    history_enc = encoder_proj.fit(
    ...
    ############### Training the Classifier with Frozen Encoder ###############
    ...
    with strategy.scope():
        # Initialize the classifier with the frozen encoder
    ...
    ############### Collecting Results for the Current Fold ###############
    print(f"#### FOLD {fold+1} OOF Accuracy = {np.max(history['val_sparse_categorical_accuracy']):.5f}")

    # Fetch validation dataset and out-of-fold predictions
    ds_valid = get_dataset(VALID_FILENAMES, ordered=True)
    oof_labels.append([target.numpy() for img, target in iter(ds_valid.unbatch())])
    x_oof = ds_valid.map(lambda image, target: image)
    oof_pred.append(np.argmax(model.predict(x_oof), axis=-1))

    # Predict features from both encoders
    oof_embed_efficientnet = encoder_efficientnet.predict(x_oof)
    oof_embed_resnet = encoder_resnet.predict(x_oof)

    # Concatenate both features
    oof_embed_combined = np.concatenate([oof_embed_efficientnet, oof_embed_resnet], axis=-1)

    # Append concatenated embeddings to oof_embed
    oof_embed.append(oof_embed_combined)

    
    # True and predicted labels for the current fold
...
# Calculate and print mean and standard deviation for each metric across all folds
...
############### Plotting Training and Validation Metrics Across All Folds ###############

plt.figure(figsize=(12, 6))
colors = ['b', 'g', 'r', 'c', 'm', 'y', 'k']  # Define different colors for each fold
...
# Concatenate the labels and predictions across all folds
y_true = np.concatenate(oof_labels)
y_pred = np.concatenate(oof_pred)
embeddings_scl = np.concatenate(oof_embed)

# Visualize embeddings using TSNE
visualize_embeddings(embeddings_scl, y_true, filename="plot_scl_combining")
...
plt.savefig("confusion_matrix_5.png", dpi=300)
plt.close()
